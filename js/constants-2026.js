(function (global) {
  ...
  const constants = {
    ...
    PROPERTY_HOLDING_TAX: {
      ...
      // 농어촌특별세법 제5조제1항: 종합부동산세액의 20%가 농어촌특별세로 함께 고지된다.
      // 재산세에는 붙지 않고 종부세에만 붙는다. 실제 납부액은 종부세 + 농특세다.
      COMPREHENSIVE_RURAL_SPECIAL_TAX_RATE: 0.2,
    },
     ...
     *
     * 간이과세자는 업종별 부가가치율을 곱해 따로 계산하므로 이 상수로는
     * 맞지 않는다. 계산기 화면에서 그 점을 밝힌다.
     */
    VAT: { RATE: 0.1 },
    ...
    PARENTAL_LEAVE_PAY: {
      // 육아휴직 기간 상한. 부모 1인당 1년이 기본이고, 부모 모두 3개월 이상 사용 등
      // 법정 요건을 갖춘 경우에 한해 1년 6개월까지 늘릴 수 있다.
      STANDARD_MAX_MONTHS: 12,
      EXTENDED_MAX_MONTHS: 18,
