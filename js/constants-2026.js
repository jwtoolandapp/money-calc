(function (global) {
  ...
  const constants = {
    ...
    YEAR_END_TAX: {
      ...
      // 기납부세액 자동 추정용 단순 비율. 실제 간이세액표(부양가족·월급 구간별)와 다르며
      // 정확도가 낮은 항목이므로 화면에 "직접 입력 권장" 안내 필수. (의도적 단순화 — 유지)
    ...
    LOTTO_TAX: {
      ...
      // 1등 평균 당첨금 프리셋 placeholder. 회차별 변동값이므로 TODO: 검증 필요
    ...
    FX: {
      // 국제브랜드 해외결제 수수료 기본값(Visa 기준 placeholder). 브랜드별 TODO: 검증 필요
      DEFAULT_BRAND_FEE_RATE: 0.011,
      // 카드사 해외이용 수수료 기본값 placeholder. 카드사·상품별 TODO: 검증 필요
      DEFAULT_ISSUER_FEE_RATE: 0.0025,
      // 현찰 스프레드 기본값 placeholder. 은행·통화·고시시점별 TODO: 검증 필요
