(function (global) {
  ...
  function cardDeduction(grossSalary, creditCard, debitCash) {
    const salary = nonNegative(grossSalary);
    ...
    const eligibleDebit = Math.max(ZERO, debit - remainingThreshold);
    const rawDeduction = eligibleCredit * TAX.CREDIT_CARD_DEDUCTION_RATE
      + eligibleDebit * TAX.DEBIT_CASH_DEDUCTION_RATE;
    const capBracket = findBracket(salary, TAX.CARD_CAP_BRACKETS, 'salaryUpTo');
    return Math.min(rawDeduction, capBracket.cap);
  }
  ...
  function calculateYearEndTax(input) {
    ...
    const personalDeduction = Math.min(
      earnedIncome,
      (TAX.TAXPAYER_COUNT + dependentCount) * TAX.PERSONAL_DEDUCTION_PER_PERSON,
    );
    const card = cardDeduction(grossSalary, input.creditCard, input.debitCash);
    ...
    const medicalEligible = Math.max(
      ZERO,
      nonNegative(input.medical) - grossSalary * TAX.MEDICAL.SALARY_THRESHOLD_RATE,
    );
    const medicalCredit = medicalEligible * TAX.MEDICAL.CREDIT_RATE;
    const educationCredit = nonNegative(input.education) * TAX.EDUCATION.CREDIT_RATE;
    ...
    const rentCredit = rentEligible
      ...
      : ZERO;
    const donation = donationCredit(input.donation);
